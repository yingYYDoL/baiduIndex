...
class BaiduIndex:
    ...
    def __init__(self, keywords: list, start_date: str, end_date: str,cookies: str, area=0):
    ...
    def _get_encrypt_datas(self, start_date, end_date, keywords):
        ...
        #
        # html = self._http_get(url, self.cookies)
    ...
    def _http_get(self, url, cookies=COOKIES):
        ...
        #代理ip
        proxy_list = [
            {"https": "http://121.8.146.99:8060"},
            {"https": "http://123.185.222.248:8118"},
            {"https": "http://59.44.78.30:42335"}
        ]
    ...
    def _decrypt_func(self, key, data):
        ...
        for index in range(len(key) // 2):
            w_data[key[index]] = key[len(key) // 2 + index]

        decrypt_data = ''
        for i in range(len(data)):
            decrypt_data += w_data[data[i]]
        return (''.join(decrypt_data).split(','))
